...
1) if no rarities specified, all the traits are set the same weight evenly, and None is not allowed; layers "Background", "Eyeball", "Eye color", "Iris", "Shine", "Bottom lid" and "Top lid" are this case;
...
4) For the 'NoneToREveal' case as mentioned above, formart {rarities: [number, [], []]} is applied, number% is the sum of percentage weight of all traits except the noneToReveal list, the two sub lists are sequentially the weight lists for except nonToReveal, and nonToReveal. Here again an empty list means weights are set evenly.
